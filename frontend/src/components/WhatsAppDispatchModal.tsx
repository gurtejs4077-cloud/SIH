...
export const WhatsAppDispatchModal: React.FC<WhatsAppDispatchModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6 bg-slate-950/75 backdrop-blur-xs overflow-y-auto"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="relative w-full max-w-4xl max-h-[88vh] bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col my-auto overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        {/* Modal Header */}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 text-xs">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
            ...
            <div className="md:col-span-5 border border-slate-200 rounded-xl p-4 bg-slate-50/50 flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-center justify-between pb-3 border-b border-slate-200">
                  ...
                  {status?.is_connected ? (
                    <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200 text-emerald-800 px-2.5 py-0.5 rounded-full text-[11px] font-bold shadow-xs">
                      <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                      <span>LINKED</span>
                    </div>
                  ) : (
                    <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-800 px-2.5 py-0.5 rounded-full text-[11px] font-bold shadow-xs">
                ...
                {status?.is_connected ? (
                  <div className="pt-4 space-y-3">
                    <div className="p-3 bg-white border border-emerald-200 rounded-xl shadow-xs space-y-2">
                      ...
                      <div className="text-slate-800 font-mono text-sm font-extrabold">
                        +{status.user_number}
                      </div>
                    ...
                    <button
                      ...
                      className="btn-secondary w-full h-8 text-xs text-rose-600 hover:text-rose-700 hover:border-rose-300"
                    >
                  ...
                  <div className="pt-3 flex flex-col items-center space-y-3">
                    {status?.qr_code ? (
                      ...
                      <div className="w-44 h-44 rounded-xl border border-dashed border-slate-300 bg-white flex flex-col items-center justify-center text-slate-400 gap-2">
                        <RefreshCw className="w-6 h-6 animate-spin text-blue-500" />
                        <span>Generating QR...</span>
                      </div>
                    )}

                    <div className="text-slate-600 text-[11px] space-y-1 w-full bg-white p-3 rounded-lg border border-slate-200 shadow-xs">
                      <strong className="text-slate-800 uppercase tracking-wide block text-[10px] mb-1">
                        Instructions:
                      </strong>
              ...
              <div className="pt-2 border-t border-slate-200/80 text-[11px] text-slate-400 text-center">
            ...
            <div className="md:col-span-7 space-y-4">
              ...
              <div className="border border-slate-200 rounded-xl p-4 bg-white shadow-xs space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  >
                    ...
                    />
                    <div>
                      <div className="font-semibold text-xs">Message Yourself</div>
                      <div className="text-[10px] text-slate-500">Linked phone number</div>
                    </div>
                  </label>
                  ...
                  >
                    ...
                    />
                    <div>
                      <div className="font-semibold text-xs">Custom Number</div>
                      <div className="text-[10px] text-slate-500">Judges or stakeholders</div>
                    </div>
                  </label>
                </div>
                ...
                {recipientType === 'custom' && (
                  <div className="pt-1">
                    ...
                    <input
                      ...
                      className="w-full h-9 px-3 rounded-lg bg-slate-50 border border-slate-200 text-xs font-mono text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="border border-slate-200 rounded-xl p-4 bg-white shadow-xs space-y-2">
                ...
                <div className="bg-slate-900 text-emerald-400 font-mono text-[11px] p-3 rounded-lg max-h-40 overflow-y-auto whitespace-pre-wrap leading-relaxed select-all">
              ...
              {/* Feedback Alerts */}
              {sendSuccess && (
                <div className="p-3 rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-800 font-medium flex items-center gap-2 shadow-xs">
              ...
              {sendError && (
                <div className="p-3 rounded-xl border border-rose-200 bg-rose-50 text-rose-800 font-medium flex items-center gap-2 shadow-xs">
